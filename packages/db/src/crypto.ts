 ...
 * needing to be a dependency of the worker.
 *
 * User password hashing (below) moved here in Phase 5 for the same
 * reason: `src/seed.ts` needs to bootstrap the first user account's
 * password hash directly (from `OWNER_PASSWORD_HASH`), without depending
 * on apps/server. apps/server/src/crypto.ts still re-exports both
 * functions below so its own call sites (the `users`/`auth` routers)
 * don't need to change where they import from.
 */
...
//
// Dependency-free, built only on `node:crypto` (no bcrypt/argon2) - this
// sandbox has no npm registry access to install and verify one against.
 ...
 *
 * Storage format: `"<saltHex>:<hashHex>"`, both hex-encoded - `user.
 * password_hash`'s only format. `OWNER_PASSWORD_HASH` (the one-time
 * `db:seed` bootstrap value for the first user account - see ./seed.ts) is
 * expected to already be in this exact format, generated by calling this
 * function once outside the running app; `verifyPassword` below is the
 * only code that needs to understand it.
 */
